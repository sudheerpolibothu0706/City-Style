...
function Login() {
  ...
  const handleResendOtp = async (emailToSend) => {
    ...
    try {
      await axios.post(`${backendUrl}/api/v1/user/forgot-password`, {
        email: emailToSend,
      });
  ...
  return (
    <div className="flex flex-col items-center justify-center mt-10 text-gray-800">
      ...
      {currentState === "Login" && (
        <form onSubmit={handleLogin} className="flex flex-col gap-4 w-[90%] sm:max-w-96">
      ...
      {currentState === "SignUp" && (
        <form onSubmit={handleSignup} className="flex flex-col gap-4 w-[90%] sm:max-w-96">
          ...
          <p className="text-xs text-gray-600">
            ⚠️ Password must be at least 8 characters and include uppercase, lowercase,
      ...
      {showOtpBox && (
        <div className="flex flex-col gap-4 mt-6 w-[90%] sm:max-w-96">
          <p className="text-sm text-gray-700">Enter the OTP sent to your email</p>
      ...
      {currentState === "Recover Password" && !showResetBox && (
        <div className="flex flex-col gap-4 w-[90%] sm:max-w-96">
          ...
          <button onClick={handleForgotPassword} className="bg-black text-white px-6 py-2">
            Send OTP
          </button>
      ...
      {showResetBox && (
        <div className="flex flex-col gap-4 mt-6 w-[90%] sm:max-w-96">
          <p className="text-sm text-gray-700">Enter OTP and your new password</p>
